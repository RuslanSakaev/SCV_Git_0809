...
__Команду `cd` набираем без команды `git`.__

15. Для работы с данным репозиторием создаём новую ветку и уже в неё вносим все изменения, после внесения изменений коммитим и отправляем на удалённый репозиторий командой `git push`.
При этом `Git` выдаст ошибку, потому что была создана новая ветка, которой небыло на удалённом репозитории. Что бы разрешить конфликт `Git` предлагает ввести команду:
```
 git push --set-upstream origin RS
```
16. Далее заходим на `Githab` и уже там появляется кнопочка `Compare & pul request`, которая позволяет отправить предложенные изменения автору проекта.
